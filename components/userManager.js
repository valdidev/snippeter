export function initUserManager(formId, listId, modalId, apiUrl, notifications) {
    ...
    if (!form || !userList || !deleteModal) return;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                notifications.showNotification(id ? 'Usuario actualizado' : 'Usuario guardado', 'info');
                resetForm();
                fetchUsers();
            } else {
                notifications.showNotification(result.error || 'Error al guardar el usuario', 'error');
            }
        } catch (error) {
    ...
    async function editUser(id) {
        try {
            ...
            if (user.error || !user.id) {
                notifications.showNotification(user.error || 'Usuario no encontrado', 'error');
                return;
            }
    ...
    confirmDeleteBtn.addEventListener('click', async () => {
        if (deleteUserId) {
            try {
                const response = await fetch(`${apiUrl}?resource=users&id=${deleteUserId}`, { method: 'DELETE' });
                const result = await response.json();
                if (response.ok) {
                    notifications.showNotification(result.mensaje, 'success');
                    fetchUsers();
                } else {
                    notifications.showNotification(result.error || 'Error al eliminar el usuario', 'error');
                }
            } catch (error) {
                notifications.showNotification('Error de red', 'error');
            }
